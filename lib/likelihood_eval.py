...
def gaussian_log_likelihood(mu_2d, data_2d, obsrv_std, indices = None):
    ...
    if n_data_points > 0:
        ...
        log_prob = gaussian.log_prob(data_2d) # Independent之后，log_prob结果为标量, batch_shape
...
def compute_binary_CE_loss(label_predictions, mortality_label):
    ...
    if len(idx_not_nan) == 0.:
        ...
        ce_loss = torch.Tensor(0.).to(utils.get_device(mortality_label))
...
def compute_multiclass_CE_loss(label_predictions, true_label, mask):
    ...
    ce_loss = torch.stack(res, 0).to(utils.get_device(label_predictions))
...
def poisson_log_likelihood(masked_log_lambdas, masked_data, indices, int_lambdas):
    ...
    #log_prob = log_prob / n_data_points
        ...
        log_prob = torch.zeros([1]).to(utils.get_device(masked_data)).squeeze()
    return log_prob
...
def mse(mu, data, indices = None):
    ...
    if n_data_points > 0:
        ...
        mse = torch.zeros([1]).to(utils.get_device(data)).squeeze()
    return mse
